...
namespace gps{
	...
	Mesh::Mesh(std::vector<Vertex> vertices, std::vector<GLuint> indices, std::vector<Texture> textures, glm::vec3 diffuseColor){
	...
	void Mesh::Draw(gps::Shader shader){
		...
		GLint colorLoc = glGetUniformLocation(shader.shaderProgram, "materialColor");
		if (colorLoc != -1){
			glUniform3fv(colorLoc, 1, glm::value_ptr(this->diffuseColor));
		}

		GLint useTexLoc = glGetUniformLocation(shader.shaderProgram, "useTexture");
		if (useTexLoc != -1){
			glUniform1i(useTexLoc, textures.size() > 0 ? 1 : 0);
		}
